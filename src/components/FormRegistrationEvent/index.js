...
import { regisEventService } from '../../services';

const FormRegistrationEvent = forwardRef(
    ({ nameEvent, tickets, sessions, updateSessionsRegisted }, ref) => {
        ...
        async function handleRegistrationEvent(e) {
            ...
            if (currentUser && typeTicket) {
                const inforRegis = {
                    ticketId: typeTicket.id,
                    sessionIds: sessionIds
                }
                const data = await regisEventService(organizer_slug, event_slug, currentUser.token, inforRegis);
                alert(data.message);
                if (data.message === 'Đăng ký thành công') {
                    ...
                    setSessionIds(() => {
                        let idOfSessionsTypeTalk =
                            sessions.filter((session) => session.type === 'talk');
